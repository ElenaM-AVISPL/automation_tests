...
describe('Login page', () => {
    ...
    it('Correct log in', () => {
        ...
        cy.get('.MuiTabs-flexContainer > :nth-child(3) > .MuiButtonBase-root > .MuiTypography-root').click({force: true})
        cy.get('legend').eq(2).should('contain.text', 'Your information')
        ...
        cy.get('legend').eq(3).should('contain.text', 'Contact information')
        ...
        cy.get('[data-value="United Kingdom"]').click({force: true})
        cy.get('legend').eq(4).should('contain.text','Preferences')
        ...
        cy.get('[data-value="(UTC-07:00) Arizona"]').click()
        cy.get('legend').eq(5).should('contain.text','Preferred Language')
        cy.get(':nth-child(4) > .MuiFormControl-root > .MuiInput-root > .MuiSelect-select').click()
        cy.get('[data-value="fr-FR"]').click()
        cy.get('.MuiInputLabel-root').eq(15).should('contain.text','Title',{force: true})
        cy.get('[type="text"]').eq(11).type('text',{force: true})
        cy.get('.MuiInputLabel-root').eq(16).should('contain.text','Department',{force: true})
        cy.get('[type="text"]').eq(12).type('state department',{force: true})
        cy.get('.MuiInputLabel-root').eq(17).should('contain.text','Supervisor',{force: true})
        cy.get('[type="text"]').eq(13).type('Supervisor', {force: true})
        cy.get('.MuiInputLabel-root').eq(18).should('contain.text','Notes', {force: true})
        cy.get('[type="text"]').eq(14).type('for notes', {force: true})
        cy.get('.MuiInputLabel-root').eq(19).should('contain.text','Billing code', {force: true})
        cy.get('[type="text"]').eq(15).type('code 432', {force: true})
        //Click Cancel
